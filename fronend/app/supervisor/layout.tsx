...
export default function SupervisorLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    async function fetchMe() {
      try {
        ...
        console.log("[SupervisorLayout] fetchMe response:", data);
        
        if (!data?.user?.role) {
          console.error("[SupervisorLayout] No user or role in response");
          router.push("/login");
          return;
        }
        
        if (data.user.role !== "SUPERVISOR") {
          console.error("[SupervisorLayout] User role is not SUPERVISOR:", data.user.role);
          router.push("/login");
          return;
        }
        
        setUser(data.user);
        console.log("[SupervisorLayout] Supervisor verified:", data.user);
      } catch (err) {
        console.error("[SupervisorLayout] Error fetching user:", err);
        router.push("/login");
      } finally {
        setLoading(false);
      }
  ...
  const navLinks = [
    ...
    { name: "My Operators", href: "/supervisor/operators", icon: Users },
    { name: "Leads", href: "/supervisor/leads", icon: TrendingUp },
    { name: "Tasks", href: "/supervisor/tasks", icon: Package },
  ];
